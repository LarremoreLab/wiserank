...
movie_recs = json.load(open(data_dir+"/recs/movie_recs.json","r"))

journal_recs = [sparse.load_npz(data_dir+'/recs/journal_recs_mx.npz'),
                json.load(open(data_dir+'/recs/journal_recs_ids.json','r'))]


def random_item(session, db):
    selected = set([str(s.obj_id) for s in session.selections])
    for _ in range(100):
        candidate = db.session.scalars(db.select(Item).filter_by(track=session.track).order_by(sqlalchemy.func.random()).limit(1)).first()
...
# custom recommendation algorithm for journals
...
def rec_journal(session,db):
    ...
    if len(distribution) > 0:
        ...
        candidate = db.session.scalars(db.select(Item).filter(and_(Item.track == session.track,
                                                                   Item.link_id == str(top_id)
                                                                   ))).first()
        return {"link_id": candidate.link_id, "name":candidate.name}
    else:
...
# custom recommendation algorithm for movies
...
def rec_item(session, db):
    # if you have a custom recommendation algorithm for your items, you can add it here
